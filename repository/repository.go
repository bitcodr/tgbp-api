...
func (repo *RepoService) GetAllDM(userID, receiverID int64) ([]*models.Message, error) {
	...
	db := app.DB()
	defer db.Close()
	rows, err := db.Query("select me.message, me.createdAt, us.customID from messages as me inner join users as us on me.userID=us.userID where type=? and ((me.userID=? and me.receiver=?) or (me.receiver=? and me.userID=?)) order by me.createdAt asc", "DM", userID, receiverID, userID, receiverID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
